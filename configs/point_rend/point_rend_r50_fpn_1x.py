...
model = dict(
    ...
    point_extractor=dict(
        type='MultiplePointExtractor', out_channels=256, featmap_strides=[4]),
    point_head=dict(
...
train_cfg = dict(
    ...
    rcnn=dict(
        ...
        debug=False),
    point=dict(
        num_points=14 * 14, oversample_ratio=3, importance_sample_ratio=0.75))
test_cfg = dict(
    rpn=dict(
        nms_across_levels=False,
        nms_pre=1000,
