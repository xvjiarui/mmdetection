...
from mmdet.core import (bbox2result, bbox2roi, build_assigner, build_sampler,
                        merge_aug_masks, merge_aug_bboxes, multiclass_nms, bbox_mapping)
from .. import builder
...
class CascadeRCNN(BaseDetector, RPNTestMixin):
    ...
    def aug_test(self, img, img_meta, proposals=None, rescale=False):
        ...
        proposal_list = self.aug_test_rpn(
            self.extract_feats(img), img_meta, self.test_cfg.rpn)

        rcnn_test_cfg = self.test_cfg.rcnn
        aug_bboxes = []
        aug_scores = []
        for x, img_meta in zip(self.extract_feats(img), img_meta):
        ...
        merged_bboxes, merged_scores = merge_aug_bboxes(
            aug_bboxes, aug_scores, img_meta, rcnn_test_cfg)
